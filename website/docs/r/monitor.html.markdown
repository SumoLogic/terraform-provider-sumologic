...
- `query_time_type` - (Optional) The time type for log monitor queries. Indicates whether the query time range is based on message time or searchable (ingestion) time. Only applicable for `Logs` monitor type. Valid values:
  - `messageTime`: Use the time the log was generated (default behavior).
  - `searchableTime`: Use the time the log was indexed/ingested.
